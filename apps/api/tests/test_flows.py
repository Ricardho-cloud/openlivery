...
def test_main_crud_knowledge_and_persistent_chat(authenticated_client: TestClient, monkeypatch):
    ...
    assert context.json()["brief_policies"] == "Does not offer emergency care."
...
def test_inbox_pagination_and_search(authenticated_client: TestClient):
    ...
    customer = client.post(
        "/api/clients",
        json={"name": "Paged Co", "is_active": True},
    ).json()
    agent = client.post(
        "/api/agents",
...
def test_dashboard_metrics(authenticated_client: TestClient):
    ...
    customer = client.post(
        "/api/clients",
        json={"name": "Metrics Co", "is_active": True},
    ).json()
    agent = client.post(
        "/api/agents",
        json={"client_id": customer["id"], "name": "Counter", "instructions": "", "personality": "", "model": "", "is_active": True},
    ).json()
    client.post("/api/conversations", json={"agent_id": agent["id"]})
    client.post("/api/conversations", json={"agent_id": agent["id"]})
...
def test_agent_brief_persists_and_reaches_prompt(authenticated_client: TestClient, monkeypatch):
    ...
    updated = client.patch(f"/api/agents/{agent['id']}", json={"brief_dos": "Confirm the pickup date"}).json()
    assert updated["brief_dos"] == "Confirm the pickup date"
    ...
    assert "Confirm the pickup date" in prompt
...
def test_custom_portal_domain_flow(authenticated_client: TestClient, monkeypatch):
    ...
    other = client.post(
        "/api/clients",
        json={"name": "Other", "is_active": True},
    ).json()
...
def test_agent_without_model_explains_configuration(authenticated_client: TestClient):
    customer = authenticated_client.post(
        "/api/clients",
        json={"name": "Client", "is_active": True},
    ).json()
    agent = authenticated_client.post(
        "/api/agents",
        json={"client_id": customer["id"], "name": "Agent", "instructions": "", "personality": "", "model": "", "is_active": True},
    ).json()
...
def test_white_label_portal_and_human_takeover(authenticated_client: TestClient):
    ...
    customer = client.post(
        "/api/clients",
        json={"name": "Luna Café", "industry": "restaurants_food", "is_active": True},
    ).json()
...
def test_portal_inbox_pages_searches_and_tracks_unread(authenticated_client: TestClient):
    ...
    customer = client.post(
        "/api/clients",
        json={"name": "Paged Portal", "is_active": True},
    ).json()
    client.post(
        f"/api/clients/{customer['id']}/portal-users",
    ...
    assert client.get(f"/api/portal/{slug}/conversations/summary").json() == {"open": 3, "resolved": 0, "archived": 0, "human": 1, "ai": 2, "unread": 1, "mine": 1, "unassigned": 0}
...
def test_portal_resolves_reopens_and_narrates_the_thread(authenticated_client: TestClient):
    ...
    customer = client.post(
        "/api/clients",
        json={"name": "Status Co", "is_active": True},
    ).json()
    client.post(
        f"/api/clients/{customer['id']}/portal-users",
...
def test_idle_ai_conversations_resolve_themselves_but_human_ones_wait(authenticated_client: TestClient):
    ...
    customer = client.post(
        "/api/clients",
        json={"name": "Idle Co", "is_active": True},
    ).json()
    agent = client.post(
        "/api/agents",
        json={"client_id": customer["id"], "name": "Host", "instructions": "", "personality": "", "model": "", "is_active": True},
