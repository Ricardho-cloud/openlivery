...
async def transcribe_audio(
    ...
    The body carries only the text and the token counts: no cost, no vendor.
    The router names the call in the ``X-Generation-Id`` header, and its
    record (with what the vendor charged) exists some ten seconds later;
    ``usage.reconcile_generation`` reads it then.
    """
    ...
    completion.generation_id = completion.generation_id or str(response.headers.get("x-generation-id") or "")
    completion.served_by = completion.served_by or str(response.headers.get("x-provider-name") or "")
    return completion
